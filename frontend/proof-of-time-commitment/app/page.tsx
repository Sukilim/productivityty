...
export default function Home() {
  const [isFirstModalOpen, setIsFirstModalOpen] = useState(false);  // State for the first modal
  const [isSecondModalOpen, setIsSecondModalOpen] = useState(false); // State for the second modal
  const [isInputDisabled, setIsInputDisabled] = useState(true); // To manage input disable/enable state

  const handleOpenModalWithDisabledInput = () => {
    setIsFirstModalOpen(true);
    setIsInputDisabled(true); // Disable input when the link is clicked
  };

  const handleOpenModalWithEnabledInput = () => {
    setIsFirstModalOpen(true);
    setIsInputDisabled(false); // Enable input when "Add Declaration" is clicked
  };
